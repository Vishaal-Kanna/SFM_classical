import numpy as np
import cv2
from utils.EstimateFundamentalMatrix import EstimateFundamentalMatrix, eight_point_F
...
def inlier_filter(Mx, My, M, n_images,w,h):
    ...
    for i in range(1, n_images):
        for j in range(i + 1, n_images + 1):
            ...
            _, inliers_a, inliers_b, inlier_index = GetInliersRANSAC(np.float32(pts1), np.float32(pts2), indices, w, h)
